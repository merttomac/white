...
-- lj-hud
local config = Config
local speedMultiplier = config.UseKMH and 2.23694 or 3.6
...
-- lj-menu 
local checklistSounds = false
local openMenuSounds = false
local resetHudSounds = false
...
RegisterNUICallback('restartHud', function()
    ...
    TriggerEvent('QBCore:Notify', "Hud yenileniyor", "error")
    ...
    Wait(2600)
    TriggerEvent('QBCore:Notify', "Hud yenilendi", "success")
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if map == "square" then 
        ...
        if showMapNotif == true then
            TriggerEvent('QBCore:Notify', 'Dikdörtgen Harita yükleniyor...')
        end
        ...
        Wait(1200)
        if showMapNotif == true then
            TriggerEvent('QBCore:Notify', 'Harita yüklendi')
        end
        elseif map == "circle" then 
            ...
            if showMapNotif == true then
                TriggerEvent('QBCore:Notify', 'Daire harita yükleniyor...')
            end
...
RegisterNetEvent("hud:client:ToggleCinematic", function()
    ...
    if CinematicModeOn == true then
        ...
        TriggerEvent('QBCore:Notify', 'Sinema modu kapalı', 'error')
        end
        DisplayRadar(1)
    elseif CinematicModeOn == false then
        ...
        if showCinematicNotif == true then
        TriggerEvent('QBCore:Notify', 'Sinema modu açık')
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
            ...
            if IsPedInAnyVehicle(ped, false) then
                if exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(PlayerPedId(), false)) <= 20 then -- At 20% Fuel Left
                    if showFuelAlert == true then
                        ...
                        TriggerEvent('QBCore:Notify', "Düşük Yakıt", "error")
